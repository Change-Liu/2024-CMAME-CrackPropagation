...
def plot_fig7a(data1, data2):
    ...
    ax.plot(filtered_data['t'], filtered_data['crack_length'], label=r'$P\rightarrow$', marker='o', markersize=16, markevery=20, lw=3, color='#008a9d')
    filtered_data = flex2[flex2['crack_length'] <= 90]
    ax.plot(filtered_data['t'], filtered_data['crack_length'], label=r'$P\leftarrow$', marker='s', markersize=16, markevery=15, lw=3, color='#eeb0b0')
    ax.plot(f0_1['t'], f0_1['crack_length'], label=r'$f_{ij}=0$', ls="--", lw=2, color='black')
...
plot_fig7a('flex1.txt', 'flex2.txt')
