...
def collect_shape_functions(element, nodal_coordinates):
    ...
    for i in range(4):
                ...
                dNdx_all=shape_function_derivatives_physical(gauss_points[i,0],gauss_points[i,1],element_coords)
                dNdx[i]=dNdx_all[:,i].T
...
def configural_force(element, N, dN, dNdx, J, v_values, G_frac):
    ...
    for i in range(4):
        ...
        for ii in range(4):
            ...
            g_frac = np.matmul((psi_frac - Gc*l*np.matmul(dv_elem,dv_elem.T)),dN[i])
...
G_frac = configural_force(connectivity[0], N, dN, dNdx, J, v, G_frac)


# Calculate the configurational force for each time step
# G_frac_time=np.zeros((2501,1))
...
for time_step in range(4001):
    ...
    print(crack_tip[time_step,0])

# # save the data
time_step=range(4001)
combined_data = np.column_stack((time_step,crack_tip_time))

# Save to a .txt file
np.savetxt("output_file.txt", crack_tip_time, fmt="%.5f")
